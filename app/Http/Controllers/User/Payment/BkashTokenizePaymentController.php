...
class BkashTokenizePaymentController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $request['payerReference'] = Auth::user()->phone;
        $request['currency'] = 'BDT';
        $request['amount'] = (int) $order_form_data['total_cost'];
        $request['merchantInvoiceNumber'] = $request->invoice;
        $request['gateway'] = "bKash";
        $request['callbackURL'] = route('bkashPayment.callBack');
        ...
        //$response =  BkashPaymentTokenize::cPayment($request_data_json,1); //last parameter 
    ...
    public function callBack(Request $request)
    {
        ...
        if ($request->status == 'success'){
            ...
            if (!$response){ 
                ...
                //$response = BkashPaymentTokenize::queryPayment($request->paymentID,1); 
            }
            ...
            if (isset($response['statusCode']) && $response['statusCode'] == "0000" && $response['transactionStatus'] == "Completed") {
                ...
                if ( isset($find) && $find->session_name == 'ecommerce_order_payment' ) {
                    ...
                    $orderId = $orderService->saveOrder($data, $response, 'bkash');
                    // $this->SaveOrder($data,$response);
    ...
    public function SaveOrder($data, $response)
    {
        ...
        $transection->transaction_number = $response['trxID'];
        $transection->paymentId = $response['paymentID'];
        $transection->formable_type = "user";
        $transection->formable_id = Auth::user()->id;
        $transection->toable_type = "admin";
        $transection->toable_id = 1;
        $transection->payment_type = "bKash";
        $transection->amount = $response['amount'];
        $transection->transection_time = $response['paymentExecuteTime'];
        ...
        return response()->json(['transectionId' => $transectionId]);
    }
    /**
