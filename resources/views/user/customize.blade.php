...
@section('user_css_content')
	<style type="text/css" media="screen">
		...
		.customization-group ul {
		    list-style: none;
		    padding: 0;
		}

		.customization-group li {
		    margin-bottom: 5px;
		}

		.customization-group li img {
...
@section('user_main_content')
		            ...
		            <div class="col-lg-6 col-md-6 col-sm-12 mb-30">
		               	<div class="product-page-content">
							...
							<div id="hover-area">
		                  	...
		                  	<p class="product-description">{{$product->short_description}}</p>
				                ...
				                <form class="single-variation-wrap" method="POST" action="{{route('cart.store')}}">
				                	@csrf
				                	@if ($product->customize_options->count())
									    @foreach ($product->customize_options->groupBy('option_type') as $optionType => $options)
									        <div class="customization-group">
									            <h3>{{ ucfirst($optionType) }} Options:</h3>
									            <ul class="option-group" data-option-type="{{ $optionType }}">
									                @foreach ($options as $option)
									                    <li>
												            <div class="form-groups">
												                <div class="custom-control custom-checkbox">
												                  	<input class="custom-control-input option-checkbox" type="checkbox" value="{{ $option->id }}" id="customization_{{ $optionType }}_{{ $option->id }}" name="customization_attribute[]" data-option-group="{{ $optionType }}">
                                

												                  	<label class="custom-control-label" for="customization_{{ $optionType }}_{{ $option->id }}">
												                  		@if ($option->image)
									                                        <img  data-enlargable src="{{Storage::disk('local')->url($option->image)}}" alt="{{ $option->option_name }}">
									                                    @endif
									...
									@endif
									<input type="hidden" name="customizeOptionPrice" value="{{$product->customize_charge}}">
									<input type="hidden" name="selected_option_hand" value="">
									<input type="hidden" name="selected_option_back" value=""> 
		                            ...
		                            <!-- Price Display -->
									<p id="price_show"></p>
									@if(isset($product->customize_charge))
										<p class="glowing">You Must pay Extra &#2547;{{$product->customize_charge}} for choose Customize Option From above</p>
									@endif
			                ...
			                <div class="product-meta">
			                    ...
			                    @endif
			                    {{-- <span>Tags: <span class="category" itemprop="category">{{str_replace(['Product', 'product'],'',$product->type->name)}}</span></span> --}}
			                </div>
		               </div>
		            </div>
		        </div>
	...
	{{-- <section class="customize_banner section-padding">
		<div class="container">
			<iframe width="100%" height="600" src="https://www.youtube.com/embed/cLOT0APQzDs?autoplay=1&mute=1"></iframe>
			<div class="cu_banner_ab">
				<div class="cu_banner_content">
					<h2>ADD YOUR MAGIC TOUCH</h2>
					<p>When all that inspiration goes into your shoe design, anything feels possible. Voilà! </p>
					<a class="custom-btn btn_button" href="single-customize.php">Customize</a>
				</div>
			</div>
		</div>
	</section> --}}
	...
	<script>
		...
		$(document).ready(function() {
		    $('.option-checkbox').click(function() {
		        var optionType = $(this).data('option-group');
		        var isChecked = $(this).is(':checked');

		        if (isChecked) {
		            // Uncheck all other checkboxes within the same option type
		            $('.option-checkbox[data-option-group="' + optionType + '"]').not(this).prop('checked', false);
		        }

		        // Maintain focus on the clicked checkbox for better user experience
		        $(this).focus();
		    });
		});
