...
@section('admin_main_content')
    ...
    <div class="container-fluid">
    	...
    	<form action="{{route('admin.product.item.update', ['product' => $product])}}" method="post" accept-charset="utf-8" enctype="multipart/form-data">
    		@csrf
        	<div class="row">
	          	...
	          	<div class="col-md-8 col-sm-6">
		                ...
		                <div class="card-body">
                            <div class="form-group">
                                ...
                                <select name="productType" id="productType" class="form-control select2tagf" required>
                                    <option value="physical" {{ $product->product_type == 'physical' ? 'select' : ''}}>Physical</option>
                                    <option value="affiliate" {{ $product->product_type == 'affiliate' ? 'select' : ''}}>Affiliate</option>
                                </select>
                            </div>
			                <div class="form-group">
			                    <label for="exampleInputEmail1">Name *</label>
			                    <input type="text"  class="form-control" onkeyup="listingslug(this.value)" id="name" value="{{ $product->name }}" name="name" placeholder="Enter Name" required>
			                </div>
			                <div class="form-group">
			                    <label for="exampleInputPassword1">Slug *</label>
			                    <input type="text" class="form-control" id="slug" value="{{ $product->slug }}"  name="slug" placeholder="Enter Slug" required>
			                </div>

                            {{-- Only show affiliate link if affiliate type --}}
                            <div class="form-group affiliate-only d-none">
                                <label>Affiliate Link *</label>
                                <input type="text" class="form-control" name="affiliate_link" id="affiliate_link" value="{{ $product->affiliate_link }}"  placeholder="Affiliate Link">
                            </div>
		                </div>
		                <!-- /.card-body -->
		            </div>
		            ...
		            <div class="card card-default">
				        ...
				        <!-- /.card-header -->
		                <div class="card-body">
		                	<img src="{{ $product->feature_image }}" class="profile-user-img img-responsive" alt="Selected Featured Image" id="output">
			                <div class="form-group">
                            ...
                            <div class="form-group" style="display: block; overflow: hidden; height: 100%;">
                                ...
                                <div id="filediv" class="row">
                                    @forelse($product->images as $image)
                                        <div id="abcd {{$loop->index + 1}}" class="col-md-3 col-sm-4 mb-3 text-center">
                                            <div class="card shadow-sm">
                                                <img id="previewimg {{$loop->index + 1}}" 
                                                    src="{{ asset($image->image_path_url) }}"  
                                                    class="card-img-top" 
                                                    style="width:100%; height:150px; object-fit:cover; border-radius:8px;" />
                                                <button type="button" class="btn btn-sm btn-danger mt-2 delete-image" data-counter="{{$loop->index + 1}}">
